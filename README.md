...
ИИ сам берёт задачи из Jira: читает описание, пишет код, проходит AI-ревью и закрывает тикет с комментарием. Это открытая часть курса — промпты и скрипты, которые настраиваются за один вечер.
...
| [scripts/review.sh](scripts/review.sh) | Тот же ревьюер через Anthropic API — для CI или запуска вне Claude Code |
