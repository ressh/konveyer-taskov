...
```bash
npx konveyer          # что это, что уже настроено и что делать дальше
npx konveyer init     # положить регламент, промпты и Makefile в свой проект
npx konveyer prompt 1 # напечатать промпт (| pbcopy — сразу в буфер)
```

`init` кладёт в проект регламент работы с Jira (`CLAUDE.md`), четыре промпта и `Makefile`.
Существующие файлы не перезаписываются: чужой `CLAUDE.md` дополняется разделом, остальное пропускается.
Глобально ничего не ставится, зависимостей у пакета нет — `npx` отрабатывает мгновенно.
